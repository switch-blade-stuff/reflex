...
namespace reflex
{
	namespace detail
	{
		...
		{
			using return_type = R;
			using arg_types = type_pack_t<Args...>;
	...
	public:
		...
		template<typename... Args>
		[[nodiscard]] auto invoke(Args &&...args) const -> std::invoke_result_t<native_function_t, void *, Args...> { return m_invoke(data(), std::forward<Args>(args)...); }
		/** @copydoc invoke */
		template<typename... Args>
		[[nodiscard]] auto operator()(Args &&...args) const -> std::invoke_result_t<native_function_t, void *, Args...> { return invoke(std::forward<Args>(args)...); }
	...
	private:
		...
		void init_obj_mem(type_pack_t<Args...>, T *instance) requires (detail::check_member<F, T *, Mem>::value && alignof(T) > flags_bits)
		{
		...
		void init_obj_mem(type_pack_t<Args...>, T &&instance) requires detail::check_member<F, std::remove_cvref_t<T>, Mem>::value
		{
