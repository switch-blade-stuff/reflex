...
namespace reflex
{
	 ...
	 * <ul>
	 * <li>`static pointer-like local_ptr()` - used to obtain local object instance pointer (usually pointer to a local static variable)</li>
	 * <li>`static pointer-like &global_ptr()` - used to obtain global object instance pointer (usually default-initialized from `local_ptr()`)</li>
	 * <li>`static Mutex &global_mtx()` - used to obtain reference to the global instance mutex.</li>
	 * </ul>
...
#ifndef REFLEX_NO_THREADS
		[[nodiscard]] inline static Mutex &invoke_global_mtx();

	public:
		/** Returns pointer to the global access guard. */
		[[nodiscard]] static auto instance() { return access_guard{invoke_global_ptr(), invoke_global_mtx()}; }
		...
		static auto instance(const auto &ptr)
		{
			...
			{
				std::lock_guard<Mutex> l{invoke_global_mtx()};
	...
	template<typename T, detail::basic_lockable Mutex>
	Mutex &guarded_instance<T, Mutex>::invoke_global_mtx()
	{
		if constexpr (requires { { T::global_mtx() } -> std::same_as<Mutex &>; })
			return T::global_mtx();
		else
		{
			static Mutex mtx;
	...
	template<typename T>
	using recursive_guarded_instance = guarded_instance<T, std::recursive_mutex>;
	/** Type alias for `guarded_instance` that uses `std::shared_mutex` as its mutex type. */
	template<typename T>
	using shared_guarded_instance = guarded_instance<T, std::shared_mutex>;
}