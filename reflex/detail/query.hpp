...
namespace reflex
{
	...
	public:
		...
		inline auto constructible_from(const argument_list &args) &&;
	...
	template<typename... Filters>
	auto type_query<Filters...>::constructible_from(const argument_list &args) && { return std::move(*this).satisfies([=](auto t) { return t.constructible_from(args); }); }
	template<typename... Filters>
