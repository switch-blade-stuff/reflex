...
namespace reflex
{
	...
	public:
		...
		inline auto constructible_from(const arg_list &args) &&;
	...
	template<typename... Filters>
	auto type_query<Filters...>::constructible_from(const arg_list &args) && { return std::move(*this).satisfies([=](auto t) { return t.constructible_from(args); }); }
	template<typename... Filters>
	...
	template<typename... Filters>
	template<typename... Args>
	auto type_query<Filters...>::constructible_from() && { return std::move(*this).constructible_from(arg_list{type_pack<Args...>}); }
